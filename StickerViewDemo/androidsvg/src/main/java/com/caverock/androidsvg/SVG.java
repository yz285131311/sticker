...
public class SVG
{
    ...
    * @return
    */
   public Picture  renderToPicture(int widthInPixels, int heightInPixels, int color)
   {
      Picture  picture = new Picture();
      Canvas   canvas = picture.beginRecording(widthInPixels, heightInPixels);
      Box      viewPort = new Box(0f, 0f, (float) widthInPixels, (float) heightInPixels);
      ...
      renderer.setSpecifiedColor(color); /** 调用自定义方法 添加渲染颜色 */
      Style.theColor = color;
      renderer.renderDocument(this, null, null, false);
   ...
   protected static class  Style implements Cloneable
   {
      ...
      public static Style  getDefaultStyle()
      {
         ...
         if(theColor != -1) {
//            def.fill = new Colour(0x123333);
            def.fill = new Colour(theColor);
         }
   ...
   protected static class Colour extends SvgPaint
   {
      ...
      {
         Log.i("--- toString",String.format("#%06x", colour));
